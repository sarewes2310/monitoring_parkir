            ...
            <div class="col">
                @if (session('status')) 
                    @if (session('status')['mode'] != 404)
                        <div class="alert alert-success" role="alert">
                            Mahasiswa <b>{{ session('status')['user'] }}</b> telah di <b>{{ session('status')['message'] }}</b>.
                        </div>
                    @else
                        <div class="alert alert-danger" role="alert">
                            Gagal melakukan aksi <b>{{ session('status')['message'] }}</b> pada mahasiswa <b>{{ session('status')['user'] }}</b>.
                        </div>
                    @endif
                @endif
                ...
                <div class="tambah-data">
                    <form class="form-inline" method="GET" action="{{ route('cariMahasiswa') }}">
                        ...
                        <div class="form-group mb-2 flex-grow-1">
                            <a class="btn btn-primary" href="{{ route('tambah_mahasiswa') }}">Tambah Member</a>
                        </div>
                        <div class="form-group mx-sm-3 mb-2">
                            <input type="text" class="form-control" id="inputCari" name="inputCari" placeholder="Nama atau NIM dari member">
                        </div>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @if ($kosong)
                            <tr>
                                <td colspan="6"> <p  class="d-flex justify-content-center">Mahasiswa Tidak Ada</p></td>
                            </tr>
                        @else    
                            @foreach ($mahasiswa as $item)                            
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <form id="edit-form" action="{{ route('edit_mahasiswa') }}" method="GET" style="display: none;">
                                            @csrf
                                            <input type="text" name="id_mahasiswa" id="id_mahasiswa" value="{{ $item->idPengguna }}">
                                        </form>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <form action="{{ route('hapus_mahasiswa') }}" method="POST">
                        @csrf
